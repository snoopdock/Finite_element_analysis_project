...
class UniversalLLMJSONParser:
    """
    A robust JSON parser designed for LLM outputs.
    ...
    Attributes:
        enable_repair: If True, use LLM to repair JSON as last resort
        repair_provider: Provider instance for LLM-based repair
        stats: Track which strategies succeed most often
    """
    ...
    def _strip_json_comments(self, text: str) -> str:
        """
        Remove JSON comments (// and /* */) while preserving strings.
        """
        result = []
        i = 0
        in_string = False
    ...
    def _normalize_quotes(self, text: str) -> str:
        ...
        FIX #8: Apostrophe-aware. Does not escape apostrophes inside double-quoted strings.
        """
        result = []
        in_string = False
        string_char = None
        ...
        for char in text:
            ...
            if char in ('"', "'"):
                if not in_string:
                    ...
                    in_string = True
                    string_char = char
                    result.append('"')  # Always open with double quote
                elif char == string_char:
                    ...
                    # FIX #8: We are inside a string and encountered the OTHER quote type.
                    # e.g., inside "..." we found ', or inside '...' we found ".
                    # Just append it as-is. Do NOT escape it with a backslash.
                    result.append(char)
            else:
                result.append(char)
    ...
    def _find_json_boundaries(self, text: str) -> Optional[Tuple[int, int]]:
        """Find outermost { } or [ ] containing balanced JSON."""
    ...
    def _detect_truncation(self, text: str) -> bool:
        """Detect if JSON appears to be truncated (cut off by max_tokens)."""
        if not text:
            return False
    ...
    def _try_candidate_extraction(self, text: str) -> Optional[Any]:
        ...
        for candidate in candidates[:5]:
            for strategy in [self._try_strict_json, self._try_normalized_json, self._try_python_literal]:
    ...
    def _try_jsonl(self, text: str) -> Optional[List[Any]]:
        """Try to parse JSON Lines (JSONL) format."""
    ...
    # ================================================================
    # PHASE 4: LLM-BASED REPAIR (with Error Feedback)
    # ================================================================
...
Return ONLY the fixed JSON. No explanation."""
        ...
        try:
            ...
            self._log(f"LLM repair exception: {e}")
            return None
    
    # ================================================================
    # PHASE 5: SCHEMA VALIDATION (Nested)
    # ================================================================
        ...
        for key, expected_type in schema.items():
            ...
            if isinstance(expected_type, dict):
                if not isinstance(actual_value, dict):
                    ...
                    is_valid, nested_errors = self._validate_schema(actual_value, expected_type)
                    if not is_valid:
                ...
                if not isinstance(actual_value, list):
                    ...
                    for i, item in enumerate(actual_value):
                        if not isinstance(item, dict):
                            ...
                            is_valid, nested_errors = self._validate_schema(item, item_schema)
                            if not is_valid:
    ...
    def parse(
        ...
        if isinstance(text, (dict, list)):
            if schema:
                is_valid, schema_errors = self._validate_schema(text, schema)
                if not is_valid:
                    raise LLMJSONParseError(
                        "Schema validation failed on pre-parsed object",
        ...
        for strategy_name, strategy_func in strategies:
            try:
                ...
                if result is not None:
                    ...
                    if schema:
                        is_valid, schema_errors = self._validate_schema(result, schema)
                        if not is_valid:
    ...
    def get_stats(self) -> Dict[str, int]:
        """Return statistics about which strategies succeeded."""
