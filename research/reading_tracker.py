...
TRACKER_FILE = REPO_ROOT / "state" / "reading_state.json"
...
def save_reading_state(state: Dict):
    if not isinstance(state, dict):
        raise TypeError("reading state must be a dictionary")

    TRACKER_FILE.parent.mkdir(parents=True, exist_ok=True)
    fd, tmp_path = tempfile.mkstemp(dir=TRACKER_FILE.parent, suffix=".tmp")
...
def mark_section_read(article_id: str, section_type: str, char_start: int, extracted_items: Dict[str, int], reading_state: Dict) -> Dict:
    if not article_id:
    ...
    article.setdefault("first_read_at", _utcnow())
    article.setdefault("read_sections", [])
    article.setdefault("section_details", [])
    article.setdefault("total_extractions", {})

    for key in ("concepts", "equations", "procedures", "rules"):
        article["total_extractions"].setdefault(key, 0)
    ...
    if section_key in article["read_sections"]:
        return reading_state
    ...
    if isinstance(extracted_items, dict):
        for key, value in extracted_items.items():
            try:
                ...
                numeric = 0
            normalized_extractions[key] = max(0, numeric)
    ...
    for key, count in normalized_extractions.items():
        if key in article["total_extractions"]:
            article["total_extractions"][key] += count

    return reading_state


def get_article_provenance(article_id: str, reading_state: Dict) -> Dict:
    if not isinstance(reading_state, dict) or article_id not in reading_state:
...
def get_unread_article_ids(reading_state: Dict, all_article_ids: List[str]) -> List[str]:
    ...
    for article_id in all_article_ids:
        if article_id not in reading_state:
            unread.append(article_id)
            continue
        article = reading_state[article_id]
        details = article.get("section_details", []) if isinstance(article, dict) else []
...
def generate_provenance_report(reading_state: Dict) -> str:
    ...
    for article_id, data in reading_state.items():
        ...
        details = data.get("section_details", [])
        ...
        for detail in details:
            ...
            section_type = str(detail.get("section_type", "unknown")).replace("_", " ").title()
            extracted = detail.get("extracted", {})
            extracted = extracted if isinstance(extracted, dict) else {}
            read_at = str(detail.get("read_at", "unknown"))[:10]
