...
def _validate_attention_proposal(proposal: Mapping[str, Any]) -> dict[str, Any]:
    ...
    condition = _require_non_empty_string(
        normalized["observed_condition"], "observed_condition"
    )
    ...
    if normalized["lifecycle_status"] != "open":
        raise ValueError(
            "R8.2 requires the canonical R7B proposal lifecycle_status to be 'open'"
        )
...
def _signal_id(
    ...
    return f"planning-signal-{digest}"
...
def _constraint(condition: str, provider: str, query_scope: str) -> dict[str, Any]:
    ...
    if condition in provider_conditions:
        return {
...
def _make_signal(proposal: Mapping[str, Any]) -> dict[str, Any]:
    ...
    signal = {
        "research_planning_signal_id": _signal_id(
            source_id,
            policy_version,
            condition,
            target,
...
def translate_attention_proposal(
...
def translate_attention_proposals(
    proposals: list[Mapping[str, Any]],
    *,
    include_created_at: bool = False,
) -> list[dict[str, Any]]:
    """Translate proposals independently, returning deterministic ordering."""
...
def validate_research_planning_signal(
    signal: Mapping[str, Any],
) -> dict[str, Any]:
    """Validate an R8.1 ResearchPlanningSignal and return a defensive copy."""
    ...
    if "translation_policy_version" in result:
        _require_non_empty_string(
            result["translation_policy_version"], "translation_policy_version"
        )
...
__all__ = [
    "ALLOWED_CONDITIONS",
    "PLANNING_SIGNAL_SCHEMA_VERSION",
    "SIGNAL_TYPE",
    "TRANSLATION_POLICY_VERSION",
    "translate_attention_proposal",
